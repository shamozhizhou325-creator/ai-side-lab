...
const explicitCover = process.argv[3] || "";
const repoRawBase =
  "https://raw.githubusercontent.com/shamozhizhou325-creator/ai-side-lab/main/";
...
const coverPath = extractBlock("封面图");
const toolLink = extractBlock("工具链接");
const record = extractBlock("发布后立刻记录");
const cover = explicitCover || (coverPath ? `${repoRawBase}${coverPath}` : "");
...
const messages = [
  ...
  toolLink ? `【5 工具链接】\n${toolLink}` : "",
  `【6 发布后记录】\n${record}`,
  "【7 安全提醒】发布前不要添加收款码、手机号、微信号、夸大收益或私密截图。",
].filter(Boolean);
